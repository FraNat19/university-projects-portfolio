#!/usr/bin/env python3
"""
✅ LAYER 1 v4: FIXED DEDUPLICATION

CRITICAL FIX:
- Usa full_citation (non law_id) per deduplicazione
- Questo mantiene TUTTE le citazioni con articoli diversi:
  - dlgs-81-2008-art4
  - dlgs-81-2008-art37
  - dlgs-81-2008 (senza articolo)
  
Tutte queste sono DIVERSE e vengono TUTTE create come CITES!
"""
...
def create_canonical_law_stubs_batch(session, laws_batch):
    """Crea stub CanonicalLaw - deduplica per law_id (corretto qui)"""
...
def create_granular_cites_relationships(session, relations_batch):
    ...
    if article_rels:
        result = session.run("""
            ...
            // CITES relationship - usa full_citation come chiave unica
            MERGE (doc)-[c:CITES {full_citation: rel.full_citation}]->(art)
            ON CREATE SET
                c.layer = 1,
                c.granularity = 'article',
                c.chunk_index = rel.chunk_index,
    ...
    if law_rels:
        result = session.run("""
            ...
            // CITES relationship - usa full_citation come chiave
            MERGE (doc)-[c:CITES {full_citation: rel.full_citation}]->(law)
            ON CREATE SET
                c.layer = 1,
                c.granularity = 'law',
                c.chunk_index = rel.chunk_index,
...
def sync_qdrant_to_neo4j_granular():
    """Main sync con CORRETTA deduplicazione per full_citation"""
    
    print(f"\n📄 Starting GRANULAR sync (FIXED deduplication)...")
    ...
    # Per CanonicalLaw, deduplica per law_id (corretto)
    seen_law_ids = set()
    ...
    with neo4j_driver.session() as session:
        ...
        while True:
            ...
            for point in points:
                ...
                # Solo summary (un doc per documento)
                ...
                # ✅✅✅ FIX CRITICO: Deduplica per full_citation, NON per law_id!
                seen_full_citations = set()
                ...
                for law in laws_structured:
                    ...
                    # ✅ Usa full_citation per deduplicazione CITES
                    full_citation = law.get('full_citation', law_id)
                    
                    if full_citation in seen_full_citations:
                        continue
                    seen_full_citations.add(full_citation)
                    unique_full_citations.add(full_citation)
                    total_citations_processed += 1
                    
                    # Per CanonicalLaw nodes, deduplica per law_id (questo è corretto)
                    ...
                    # ✅ RELATIONS con granularità - OGNI full_citation unica
                    ...
                    relation_data = {
                        ...
                        'chunk_index': 0,
                        'citation_context': law.get('context', 'document-level')[:200] if law.get('context') else 'document-level',
    ...
    print(f"   CanonicalLaw nodes: {created_laws:,}")
    ...
    print(f"   Unique full_citations: {len(unique_full_citations):,}")
...
def verify_sync():
    ...
    with neo4j_driver.session() as session:
        ...
        print(f"📄 TechnicalDocument: {docs:,}")
        
        result = session.run("""
            MATCH (l:CanonicalLaw)
            RETURN count(l) as laws
        """)
        laws = result.single()['laws']
        print(f"⚖️  CanonicalLaw: {laws:,}")
        
        result = session.run("""
            MATCH (a:Article)
            RETURN count(a) as arts
        """)
        arts = result.single()['arts']
        print(f"📜 Article: {arts:,}")
        ...
        result = session.run("""
            MATCH (art:Article)
            WHERE art.from_qdrant = true OR art.from_xml = true
            RETURN count(art) as cnt
        """)
        ...
        # Check same law_id with different articles
        result = session.run("""
            MATCH (doc:TechnicalDocument)-[c:CITES]->(art:Article)
            WITH doc, art.parent_law_id as law_id, count(c) as citations
            WHERE citations > 1
            RETURN doc.doc_id as doc, law_id, citations
            ORDER BY citations DESC
            LIMIT 5
        """)
        
        print("\n📋 Docs citing same law with multiple articles:")
        found = False
        for rec in result:
            found = True
            print(f"   {rec['doc'][:40]}... → {rec['law_id']}: {rec['citations']} different articles")
        
        if not found:
            print("   (none found - this might indicate a problem if you expected granular citations)")
        ...
        result = session.run("""
            ...
            RETURN 
                ...
                c.granularity as granularity,
                c.full_citation as full_citation,
        ...
        for rec in result:
            ...
            print(f"      full_citation: {rec['full_citation']}")
            if rec['citation']:
                print(f"      cited_as: {rec['citation'][:50]}...")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--sync', action='store_true', help='Sync with FIXED deduplication')
    ...
    if not any([args.sync, args.verify]):
        print("\nUsage:")
        print("  --sync    : Sync with FIXED full_citation deduplication")
